...
const OrderDetails = ({ token: propToken }) => {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      ...
      setLoading(true);
      try {
        // Backend contract: GET /retailer/orders/{id}
        const data = await api.get(`/user/retailer/orders/${id}`, token);
        setOrder(data);
  ...
  const handlePayment = async () => {
    ...
    setPaying(true);
    try {
      // Backend contract: PATCH /retailer/orders/{id}/pay
      const updated = await api.patch(`/retailer/orders/${id}/pay`, {}, token);
      setOrder(updated || { ...order, paymentStatus: "PAID" });
      toast.success("Payment marked as completed");
    } catch (err) {
      toast.error(err.message || "Payment action failed");
    } finally {
      setPaying(false);
    }
  };
        ...
        <div className="detail-left">
          ...
          <div className="detail-actions">
            <button
              ...
              disabled={paying || (order.paymentStatus || "").toUpperCase() === "PAID" || (order.status || "").toUpperCase() !== "CONFIRMED"}
            >
