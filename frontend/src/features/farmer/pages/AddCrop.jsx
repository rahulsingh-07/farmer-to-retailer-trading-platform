...
import { addCrop as addCropApi } from '../../farmer/farmerService';
...
export default function AddCrop() {
    ...
    const cropTypes = useMemo(() => ["AUCTION", "FIXED"], []);
    ...
    const validate = () => {
        ...
        const ALLOWED_CROP_TYPES = ["AUCTION", "FIXED"];

        if (!ALLOWED_CROP_TYPES.includes(formData.cropType)) {
            nextErrors.cropType = "Crop type must be AUCTION or FIXED";
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await addCropApi(formDataToSend);
    ...
    return (
        <div className="add-crop-page">
            ...
            <div className="add-crop-card">
                {message && (
                    <div
                        className={`banner ${message.toLowerCase().includes("unable") ? "error" : "success"
                            }`}
                    >
                        {message}
                    </div>
